...
const addListener=(element,eventName,callback)=>{
  document.querySelector(element).addEventListener(eventName,callback)
}

const fetch=(url,renderCBFunction)=>{
 ...
 e.preventDefault();
  ...
  fetch(url,function (response) {
    document.querySelector('#results-summary').innerHTML =
      `<p>Query of ${tags} returned ${response.items.length} results</p>`;
...
addListener('#form-answerers', 'submit', function (e) {
  ...
  fetch(url,function (response) {
     document.querySelector('#results-summary').innerHTML = 
         `<p>Query of ${tag} returned ${response.items.length} results</p>`;
